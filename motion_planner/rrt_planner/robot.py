...
class Robot:
  ...
  def distance(self, configuration): # configuration distance
    return sqrt(configuration[0]*configuration[0] + configuration[1]*configuration[1]) + \
...
class RobotArm:
  ...
  def distance(self, configuration): # configuration distance
    assert len(configuration) == len(self.joints)
    return max([abs(configuration[i])*self.joints[i][1].get_radius() \
                for i in range(len(configuration))])
...
class Range:                    # range of values, handles wraparound
...
class ConfigurationSpace:       # Cspace for robot (ranges and distance)
  def __init__(self, cspace_ranges, start, robot_distance, max_steps):
    self.cspace_ranges = cspace_ranges
    self.robot_distance = robot_distance
    self.max_diff_on_path = max_steps

  def distance(self, one, two): # distance in Cspace
    return self.robot_distance(tuple([self.cspace_ranges[i].difference(one[i], two[i]) \
                                      for i in range(len(self.cspace_ranges))]))

  def path(self, one, two):     # linear interpolation
    assert self.valid_configuration(one) and self.valid_configuration(two)
    diffs = [self.cspace_ranges[i].difference(one[i], two[i]) \
             for i in range(len(self.cspace_ranges))]
    ...
    for s in range(1, samples-1):
      sample = tuple([self.cspace_ranges[i].in_range(one[i] + s*linear_interpolation[i]) \
                      for i in range(len(self.cspace_ranges))])
      #  return path
      path.append(sample)
    return path + [two]
