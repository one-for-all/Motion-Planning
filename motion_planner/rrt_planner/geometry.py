...
class Line:
  ...
  def cut(self, other): # Cut one line with another, return the (up to) two segments
    if not self.get_aabb().collides(other.get_aabb()):
      return None
    d_1 = (self.end - self.start)
    perp_1 = -d_1.perpendicular()
  ...
  def draw(self, window, color="black", width = 3): # draw
    window.drawLineSeg(self.start.x, self.start.y,
                       self.end.x, self.end.y, color=color, width=width)
...
class AABB:
  ...
  def get_verts(self):          # vertices of box
    if self.verts is None:
      self.verts = [self.min, Point(self.max.x, self.min.y),
                    self.max, Point(self.min.x, self.max.y)]
    return self.verts
  ...
  def contains(self, other, edges = False): # is some entity contained in the box
    ...
    if isinstance(other, Point):
      ...
      if edges:
        return self.min.x <= other.min.x and self.min.y <= other.min.y and \
          self.max.x >= other.max.x and self.max.y >= other.max.y
      else:
        return self.min.x < other.min.x and self.min.y < other.min.y and \
          self.max.x > other.max.x and self.max.y > other.max.y  
...
class Polygon:
  ...
  def get_lines(self):          # get edge lines
    return [Line(self.points[i-1], self.points[i]) \
            for i in range(len(self.points))]
    ...
    if isinstance(other, Point): # point in polygon
      if edges:
        return self.get_aabb().contains(other) and \
          all([line[0].dot(other) + line[1] <= 0 for line in self.get_leqs()])
      else:
        return self.get_aabb().contains(other) and \
          all([line[0].dot(other) + line[1] < 0 for line in self.get_leqs()])
  ...
  def sweep_move(self, direction): # sweep convex polygon along direction (point)
    return [Polygon([self.points[i-1], self.points[i],
                     self.points[i] + direction, self.points[i-1] + direction]) \
            for i in range(len(self.points))]
  ...
  def __eq__(self, other):
    return isinstance(other, Polygon) and len(self.points) == len(other.points) and \
      all(self.points[i] == other.points[i] for i in range(len(self.points)))
...
class Object:
  def __init__(self, reference, polys):
    ...
    self.polys = [poly.translate(reference) for poly in polys] # in global coords
    self.aabb = None
    self.radius = None
  ...
  def sweep_move(self, start, finish): # swept volume
    return Object(start, [swept for poly in self.local_polys \
